...
juce::AudioProcessorValueTreeState::ParameterLayout FiveParksVST3AudioProcessor::createParameters()
{
    ...
    p.push_back(pc("noise_type", "Noise Type", { "White", "Air", "Metal", "Crunch", "Vowel", "Laser", "Horn", "Vinyl" }, 0));
    ...
    p.push_back(pc("filter_mode", "Filter Mode", { "LP", "BP", "HP", "Notch", "Peak" }, 0));
    ...
    p.push_back(pc("bass_mode", "Bass Mode", { "Init", "Reese", "Womp", "Screech", "Warhorn", "Whoop", "Donk", "Tearout", "Laser", "Neuro", "Horn Stab", "Vapor" }, 0));
    p.push_back(pf("reese_amt", "Reese", 0.0f, 1.0f, 0.32f));
    p.push_back(pf("warhorn_amt", "Warhorn", 0.0f, 1.0f, 0.18f));
    p.push_back(pf("womp_amt", "Womp", 0.0f, 1.0f, 0.24f));
    p.push_back(pf("subharm_amt", "Sub Harm", 0.0f, 1.0f, 0.30f));
    p.push_back(pf("combo_amt", "Combo", 0.0f, 1.0f, 0.22f));
    p.push_back(pf("stereo_width", "Stereo Width", 0.0f, 1.0f, 0.62f));
    p.push_back(pf("fm_amt", "FM Grit", 0.0f, 1.0f, 0.16f));
    p.push_back(pf("air_amt", "Air", 0.0f, 1.0f, 0.10f));
    p.push_back(pf("body_amt", "Body", 0.0f, 1.0f, 0.24f));
    p.push_back(pf("growl_amt", "Growl", 0.0f, 1.0f, 0.16f));
    p.push_back(pf("tone_amt", "Tone", 0.0f, 1.0f, 0.50f));
    p.push_back(pf("motion_amt", "Motion", 0.0f, 1.0f, 0.18f));
    p.push_back(pf("screech_amt", "Screech", 0.0f, 1.0f, 0.12f));
    p.push_back(pf("whoop_amt", "Whoop", 0.0f, 1.0f, 0.10f));
    p.push_back(pf("vowel_amt", "Vowel", 0.0f, 1.0f, 0.15f));
    p.push_back(pf("smear_amt", "Smear", 0.0f, 1.0f, 0.12f));
...
void FiveParksVST3AudioProcessor::allNotesOff(bool hardReset)
{
    heldNotes.clear();
    sustainedNotes.clear();
    sustainPedalDown = false;
    for (auto& vel : noteVelocities) vel = 0.0f;
    for (auto& v : voices)
    {
        if (hardReset) v.forceKill();
        else if (v.isActive()) v.stop();
    }
}

int FiveParksVST3AudioProcessor::chooseHeldNote() const
{
    if (heldNotes.empty()) return -1;
    return heldNotes.back();
}

void FiveParksVST3AudioProcessor::handleSustainPedal(bool down)
{
    ...
    auto released = sustainedNotes;
    sustainedNotes.clear();
    for (int midiNote : released)
        noteOff(midiNote);
...
void FiveParksVST3AudioProcessor::noteOn(int midiNote, float velocity)
{
    ...
    p.noisePhase = getF(apvts, "noise_phase");

    noteVelocities[(size_t) juce::jlimit(0, 127, midiNote)] = velocity;
    ...
    if (mode != 2)
    {
        ...
        const bool retrigger = (mode == 0) || ! voices[(size_t) idx].isActive();
        if (voices[(size_t) idx].isActive())
            voices[(size_t) idx].retarget(midiNote, velocity, p, ++voiceCounter, retrigger);
        else
...
{
    heldNotes.erase(std::remove(heldNotes.begin(), heldNotes.end(), midiNote), heldNotes.end());
    noteVelocities[(size_t) juce::jlimit(0, 127, midiNote)] = 0.0f;

    if (sustainPedalDown)
    {
        sustainedNotes.erase(std::remove(sustainedNotes.begin(), sustainedNotes.end(), midiNote), sustainedNotes.end());
        sustainedNotes.push_back(midiNote);
        return;
    }

    sustainedNotes.erase(std::remove(sustainedNotes.begin(), sustainedNotes.end(), midiNote), sustainedNotes.end());

    const int mode = getI(apvts, "play_mode");
    if (mode == 2)
    {
        for (auto& v : voices)
            if (v.isActive() && v.getMidiNote() == midiNote)
                v.stop();
        if (heldNotes.empty() && sustainedNotes.empty())
            allNotesOff(true);
        return;
    }
    ...
    int activeIdx = -1;
    for (int i = 0; i < maxVoices; ++i)
        if (voices[(size_t) i].isActive()) { activeIdx = i; break; }
    ...
    if (! heldNotes.empty())
    {
        ...
        const int nextNote = chooseHeldNote();
        const float vel = noteVelocities[(size_t) juce::jlimit(0, 127, nextNote)] > 0.0f ? noteVelocities[(size_t) juce::jlimit(0, 127, nextNote)] : 1.0f;
        voices[(size_t) activeIdx].retarget(nextNote, vel, p, ++voiceCounter, mode == 0);
    }
    else
    {
        for (auto& v : voices)
...
void FiveParksVST3AudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    p.noiseType = getI(apvts, "noise_type"); p.noisePitch = getF(apvts, "noise_pitch"); p.noisePhase = getF(apvts, "noise_phase"); p.noisePan = getF(apvts, "noise_pan"); p.noiseLevel = getF(apvts, "noise_level"); p.noiseDirect = getB(apvts, "noise_direct");
    ...
    p.bassMode = getI(apvts, "bass_mode");
    p.reeseAmount = getF(apvts, "reese_amt"); p.warhornAmount = getF(apvts, "warhorn_amt"); p.wompAmount = getF(apvts, "womp_amt");
    p.subHarmonic = getF(apvts, "subharm_amt"); p.comboAmount = getF(apvts, "combo_amt"); p.stereoWidth = getF(apvts, "stereo_width");
    p.fmAmount = getF(apvts, "fm_amt"); p.airAmount = getF(apvts, "air_amt");
    p.bodyAmount = getF(apvts, "body_amt"); p.growlAmount = getF(apvts, "growl_amt"); p.toneAmount = getF(apvts, "tone_amt"); p.motionAmount = getF(apvts, "motion_amt");
    p.screechAmount = getF(apvts, "screech_amt"); p.whoopAmount = getF(apvts, "whoop_amt"); p.vowelAmount = getF(apvts, "vowel_amt"); p.smearAmount = getF(apvts, "smear_amt");
    ...
    for (const auto metadata : midi)
    {
        ...
        {
            handleSustainPedal(msg.getControllerValue() >= 64);
        }
    ...
    if (! sustainPedalDown && heldNotes.empty() && sustainedNotes.empty())
    {
        for (auto& v : voices)
            if (v.isActive())
                v.forceKill();
    ...
    for (int sample = 0; sample < numSamples; ++sample)
    {
        ...
        chorusPhase += (0.18f + p.reeseAmount * 0.21f) / (float) currentSampleRate;
        if (chorusPhase >= 1.0f) chorusPhase -= 1.0f;
        const float chorusMod = 0.003f + (0.002f + p.stereoWidth * 0.002f) * std::sin(chorusPhase * juce::MathConstants<float>::twoPi);
        ...
        const int delaySamps = (int) ((0.24 + 0.18 * p.wompAmount) * currentSampleRate);
        ...
        delayLineL[(size_t) delayWrite] = l + dL * (0.28f + 0.22f * p.delayMix);
        delayLineR[(size_t) delayWrite] = r + dR * (0.28f + 0.22f * p.delayMix);
    ...
    reverbParams.roomSize = 0.30f + 0.55f * p.reverbMix + 0.10f * p.comboAmount;
    reverbParams.damping = 0.18f + 0.50f * p.reverbMix;
    reverbParams.width = juce::jlimit(0.0f, 1.0f, 0.55f + 0.45f * p.stereoWidth);
    reverbParams.freezeMode = 0.0f;
    reverbParams.wetLevel = juce::jlimit(0.0f, 1.0f, p.reverbMix * 0.34f);
    reverbParams.dryLevel = 1.0f;
    reverb.setParameters(reverbParams);
    if (p.reverbMix > 0.0001f)
    ...
    for (auto& v : voices)
    {
        ...
        {
            ++activeVoiceCount;
            const bool noteStillHeld = std::find(heldNotes.begin(), heldNotes.end(), v.getMidiNote()) != heldNotes.end();
            const bool noteSustained = sustainPedalDown && std::find(sustainedNotes.begin(), sustainedNotes.end(), v.getMidiNote()) != sustainedNotes.end();
            if (! noteStillHeld && ! noteSustained && v.getVelocity() <= 0.0001f)
                v.forceKill();
        }
    }
    ...
    for (const auto& v : voices)
        if (v.isActive()) { anyActive = true; break; }
    if (! anyActive && ! heldNotes.empty())
        heldNotes.clear();
