...
namespace
{
    ...
    Colour panel()     { return Colour(0xff120f2e); }
    Colour panel2()    { return Colour(0xff071d39); }
    ...
    void drawCard(Graphics& g, Rectangle<float> r, const String& title)
    {
        for (int i = 0; i < 5; ++i)
        {
            g.setColour(accent().interpolatedWith(accent2(), i / 4.0f).withAlpha(0.020f * (5 - i)));
            g.drawRoundedRectangle(r.expanded((float) i * 1.5f), 16.0f + i, 2.2f - i * 0.25f);
        }

        ColourGradient shell(panel2(), r.getTopLeft(), panel(), r.getBottomRight(), false);
        shell.addColour(0.22, Colour(0xff32104f));
        shell.addColour(0.48, Colour(0xff003855));
        shell.addColour(0.74, Colour(0xff40153e));
        shell.addColour(1.0, Colour(0xff09131d));
        g.setGradientFill(shell);
        g.fillRoundedRectangle(r, 16.0f);

        auto glowStrip = r.reduced(2.0f);
        glowStrip.setHeight(38.0f);
    ...
    StringArray bassModeItems(){ return { "Init", "Reese", "Womp", "Screech", "Warhorn", "Whoop", "Donk", "Tearout" }; }
    StringArray noiseTypeItems(){ return { "White", "Air", "Metal", "Crunch", "Vowel" }; }
    ...
    : AudioProcessorEditor(&p), audioProcessor(p), scope(p), spectrum(p), lfoShape(p), matrixView(p)
{
    ...
    logo.setText("5parks VST3 · RAINBOW MONSTER", dontSendNotification);
    logo.setColour(juce::Label::textColourId, textHi());
    logo.setFont(juce::FontOptions(26.0f, juce::Font::bold));
    subtitle.setText("dual oscillators · selectable bass characters · rainbow neon shell · whoops · womps · screeches · warhorns", dontSendNotification);
...
{
    ColourGradient bgGrad(Colour(0xff180020), 0.0f, 0.0f, Colour(0xff00192e), (float) getWidth(), (float) getHeight(), false);
    bgGrad.addColour(0.16, Colour(0xff4c0f8c));
    bgGrad.addColour(0.34, Colour(0xff0d5fff));
    bgGrad.addColour(0.52, Colour(0xff00d1b0));
    bgGrad.addColour(0.70, Colour(0xffff5c8a));
    bgGrad.addColour(0.86, Colour(0xffffa63b));
    bgGrad.addColour(1.00, Colour(0xff1a0f39));
    g.setGradientFill(bgGrad);
    g.fillAll();

    for (int i = 0; i < 10; ++i)
    {
        const float y = (float) getHeight() * (0.06f + 0.085f * (float) i);
        Colour c0 = (i % 4 == 0 ? accent() : (i % 4 == 1 ? accent2() : (i % 4 == 2 ? accent3() : accent4())));
        Colour c1 = (i % 4 == 0 ? accent4() : (i % 4 == 1 ? accent() : (i % 4 == 2 ? accent2() : accent3())));
        ColourGradient beam(c0.withAlpha(0.07f + 0.01f * i), 0.0f, y, c1.withAlpha(0.01f), (float) getWidth(), y + 54.0f, false);
        g.setGradientFill(beam);
        g.fillRect(0, (int) y, getWidth(), 48);
    }
    ...
    auto kb = getLocalBounds().removeFromBottom(38).reduced(26, 0).toFloat();
...
void FiveParksVST3AudioProcessorEditor::resized()
{
    ...
    filterBox.setBounds(middle.removeFromLeft(250));
    middle.removeFromLeft(8);
    bassBox.setBounds(middle.removeFromLeft(430));
    middle.removeFromLeft(8);
    envBox.setBounds(middle.removeFromLeft(250));
    middle.removeFromLeft(8);
    lfoBox.setBounds(middle.removeFromLeft(250));
    middle.removeFromLeft(8);
