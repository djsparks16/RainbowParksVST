...
struct RuntimeParams
{
    ...
    int noiseType = 0; float noisePitch = 0.5f, noisePhase = 0.0f, noisePan = 0.5f, noiseLevel = 0.0f; bool noiseDirect = false;
...
struct WavetableOscillator
{
    ...
    float frame(float p, float pos) const
    {
        ...
        return juce::jmap((pos - 0.90f) / 0.10f, formant, folded);
    }
...
public:
    ...
    void reset()
    {
        ...
        age = 0; basePan = 0.5f; releaseSamples = 0;
    }
    ...
    {
        active = true; held = true; midiNote = note; velocity = vel; age = newAge; releaseSamples = 0;
    ...
    StereoSample render(const RuntimeParams& p)
    {
        ...
        float modeReese = 0.0f, modeWomp = 0.0f, modeScreech = 0.0f, modeWarhorn = 0.0f, modeWhoop = 0.0f, modeDonk = 0.0f, modeTear = 0.0f;
        switch (p.bassMode)
        {
        ...
        const float motion = juce::jlimit(-1.0f, 1.0f, lfoValues[0] * (0.45f + 0.9f * p.motionAmount + 0.55f * macroWomp) + lfoValues[1] * (0.35f * p.motionAmount + 0.18f * macroWhoop) + env3v * 0.2f * p.motionAmount);
        ...
        const auto oscOutA = renderStack(oscA, extraA, p.oscAUnison + p.comboAmount * 2.0f, p.oscADetune + macroReese * 0.25f, p.oscABlend, juce::jlimit(0.0f, 1.0f, p.oscAPan + modPan * 0.2f - macroReese * 0.15f), true);
        const auto oscOutB = renderStack(oscB, extraB, p.oscBUnison + p.comboAmount * 2.0f, p.oscBDetune + macroReese * 0.25f, p.oscBBlend, juce::jlimit(0.0f, 1.0f, p.oscBPan - modPan * 0.2f + macroReese * 0.15f), false);
        ...
        if (p.subHarmonic > 0.001f)
        {
            const float sub2 = std::sin(sub.phase * juce::MathConstants<float>::twoPi * 0.5f);
            subSample += sub2 * (0.55f * p.subLevel * p.subHarmonic);
        }

        float noiseRaw = noise.process() * p.noiseLevel;
        const float nphase = noise.phase;
        if (p.noiseType == 1)
            noiseRaw = (0.55f * noiseRaw + 0.45f * std::sin(juce::MathConstants<float>::twoPi * nphase * 12.0f)) * p.noiseLevel;
        else if (p.noiseType == 2)
            noiseRaw = (0.35f * noiseRaw + 0.65f * std::sin(juce::MathConstants<float>::twoPi * nphase * (21.0f + 7.0f * env3v))) * p.noiseLevel;
        else if (p.noiseType == 3)
            noiseRaw = std::tanh((noiseRaw + std::sin(juce::MathConstants<float>::twoPi * nphase * 5.0f)) * 2.5f) * p.noiseLevel;
        else if (p.noiseType == 4)
            noiseRaw = (0.45f * noiseRaw + 0.55f * std::sin(juce::MathConstants<float>::twoPi * (nphase + 0.08f * env2v) * (3.0f + 8.0f * p.vowelAmount))) * p.noiseLevel;
        ...
        l += comboMono * p.comboAmount * 0.35f;
        r += comboMono * p.comboAmount * 0.35f;

        if (! p.subDirect) { l += subSample; r += subSample; }
        if (! p.noiseDirect) { l += noiseRaw * noisePanL; r += noiseRaw * noisePanR; }

        const float toneTilt = juce::jmap(p.toneAmount, 0.0f, 1.0f, 0.32f, 2.85f);
        const float toneSweep = 1.0f + (p.motionAmount * 0.15f + macroWomp * 0.35f + macroWhoop * 0.20f) * lfoValues[2] + p.growlAmount * 0.10f * env2v + p.vowelAmount * 0.18f * env3v;
        ...
        const float preDrive = 1.0f + p.filterDrive * (4.0f + p.growlAmount * 1.5f) + p.toneAmount * 0.8f;
        ...
        if (macroWarhorn > 0.001f)
        {
            const float hornL = std::sin(l * (4.0f + 12.0f * macroWarhorn) + env2v * 3.0f + motion * 2.0f) * 0.35f;
            const float hornR = std::sin(r * (4.0f + 12.0f * macroWarhorn) + env2v * 3.0f - motion * 2.0f) * 0.35f;
            l = std::tanh(l + hornL + (p.fmAmount + macroScreech * 0.3f) * oscB.process(0.17f) * 0.15f);
            r = std::tanh(r + hornR + (p.fmAmount + macroScreech * 0.3f) * oscA.process(-0.11f) * 0.15f);
        }

        if (p.growlAmount > 0.001f)
        {
            const float ringL = std::sin((l + env2v * 0.4f) * (6.0f + 18.0f * p.growlAmount) + oscA.process(0.03f) * 1.7f);
            const float ringR = std::sin((r + env2v * 0.4f) * (6.0f + 18.0f * p.growlAmount) + oscB.process(-0.02f) * 1.7f);
        ...
        const float distDrive = p.distDrive + modDist * 0.35f + macroWarhorn * 0.25f + p.fmAmount * 0.2f + p.growlAmount * 0.35f + p.toneAmount * 0.10f + macroScreech * 0.25f + macroWhoop * 0.15f;
        const float foldAmt = juce::jlimit(0.0f, 1.0f, p.motionAmount * 0.55f + p.growlAmount * 0.35f + macroWarhorn * 0.25f);
        ...
        if (macroScreech > 0.001f)
        {
            const float screechL = std::sin((l + oscA.process(0.13f) * 0.4f) * (8.0f + 26.0f * macroScreech) + lfoValues[3] * 2.0f);
            const float screechR = std::sin((r + oscB.process(-0.16f) * 0.4f) * (8.0f + 26.0f * macroScreech) - lfoValues[3] * 2.0f);
            l = juce::jmap(0.35f * macroScreech, l, std::tanh(l + screechL * 0.55f));
            r = juce::jmap(0.35f * macroScreech, r, std::tanh(r + screechR * 0.55f));
        }
        if (macroWhoop > 0.001f || p.vowelAmount > 0.001f)
        {
        ...
        const float smearTap = std::sin((oscA.phase + oscB.phase) * juce::MathConstants<float>::twoPi * (1.0f + 4.0f * p.smearAmount)) * p.smearAmount * 0.14f;
        l += smearTap;
        r -= smearTap;
        ...
        l = mid + side;
        r = mid - side;
        const float air = (p.airAmount + 0.35f * (p.toneAmount - 0.5f)) * 0.12f;
        ...
        if (! held)
        {
            ...
            if (releaseSamples > maxReleaseSamples || (! env1.isActive() && std::abs(l) + std::abs(r) < 0.00002f))
                reset();
        }
        else
        {
