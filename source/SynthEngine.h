...
public:
    ...
    StereoSample render(const RuntimeParams& p)
    {
        ...
        if (p.noiseType == 1)
            ...
            noiseRaw = (0.20f * noiseRaw + 0.80f * std::sin(juce::MathConstants<float>::twoPi * nphase * (36.0f + 22.0f * macroLaser))) * p.noiseLevel;
        else if (p.noiseType == 6)
            noiseRaw = std::tanh((noiseRaw + std::sin(juce::MathConstants<float>::twoPi * nphase * (9.0f + 6.0f * macroHornStab))) * 2.2f) * p.noiseLevel;
        else if (p.noiseType == 7)
            noiseRaw = (0.75f * noiseRaw + 0.25f * std::sin(juce::MathConstants<float>::twoPi * nphase * 1.1f)) * p.noiseLevel;
        ...
        if (macroWarhorn > 0.001f || macroHornStab > 0.001f)
        {
        ...
        l = mid + side;
        r = mid - side;
        const float air = (p.airAmount + 0.35f * (p.toneAmount - 0.5f) + 0.25f * macroLaser + 0.20f * macroVapor) * 0.12f;
